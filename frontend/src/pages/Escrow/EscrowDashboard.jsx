...
const EscrowDashboard = () => {
  ...
  const columns = [
    ...
    { 
      ...
      render: (name) => (
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded bg-trust-teal/10 flex items-center justify-center text-trust-teal">
            <ShieldCheck size={14} />
          </div>
    ...
    { 
      header: 'Last Update', 
      accessor: 'date',
      render: (date) => <span className="text-[11px] text-text-muted font-mono">{date}</span>
    },
    {
      header: 'Actions',
      accessor: 'id',
      render: () => (
        <ActionMenu actions={[
          ...
          { label: 'Release Funds', icon: <ArrowUpRight size={14} />, onClick: () => {} },
          { label: 'Raise Dispute', icon: <AlertCircle size={14} />, variant: 'danger', onClick: () => {} }
        ]} />
      )
    }
  ];
    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-bold font-display text-text-primary tracking-tight">Escrow Accounts</h1>
          <p className="text-text-muted text-sm">Securely manage your transaction funds and approvals.</p>
        </div>
      ...
      <footer className="bg-trust-teal/5 border border-trust-teal/20 rounded-card p-6 flex flex-col md:flex-row items-center gap-6">
        <div className="w-12 h-12 rounded-full bg-trust-teal text-page-bg flex items-center justify-center shrink-0">
          <ShieldCheck size={28} />
        </div>
        <div className="space-y-1 flex-1 text-center md:text-left">
          <h4 className="font-bold text-text-primary uppercase tracking-wider text-sm">RBI Regulated Security</h4>
        ...
        <Button variant="ghost" size="sm" className="text-trust-teal hover:bg-trust-teal/5">
